...
class _CategoriesScreenState extends State<CategoriesScreen> {
  Future<void> _showAddDialog() async {
    ...
    if (result != null && result.isNotEmpty) {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Category "$result" added')));
    }
  }
  ...
  Future<void> _showRenameDialog(String currentName) async {
    ...
    if (result != null && result.isNotEmpty) {
      ...
      final idx = cats.indexWhere((c) => c.toLowerCase() == currentName.toLowerCase());
      if (idx != -1) {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('"$currentName" renamed to "$result"')));
    }
  }
              ...
              return ListView.separated(
                ...
                itemBuilder: (ctx, i) {
                  ...
                  final itemsForCat = repoItems
                      .where((it) => (it?.category ?? '').toLowerCase() == name.toLowerCase())
                      .toList();
